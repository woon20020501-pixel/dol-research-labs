...
*Test vectors are generated by the Python reference implementation in `polyvault_defi_sim.py` after both corpus hashes are pinned. Once pinned, all implementations (Python, Rust, TypeScript) MUST produce bit-identical `he_blob`s for the following table.*

### 9.1 Placeholder table (fill at first release)

| he_key (hex, 32B) | passphrase | nonce (hex, 12B) | he_blob (hex, 22B) | decode(wrong_key) output |
|---|---|---|---|---|
| `0000…0000` | `"password"` | `000…000` | `<compute>` | `<compute>` |
| `0000…0000` | `"correcthorse"` | `000…000` | `<compute>` | `<compute>` (tail) |
| `ffff…ffff` | `"123456"` | `fff…fff` | `<compute>` | `<compute>` |

Once filled, these vectors are a regression gate — any implementation change that alters the output for any vector is a breaking change that requires a `dte_variant` bump.
