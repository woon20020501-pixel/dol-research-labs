...
2. **No operator signature suffices to move user funds.** Dol's own keys (the paymaster key, any emergency pause role) cannot transfer user balances or change the owner. The contract's access control enforces this.
...
When the marketing copy says "non-custodial," the five bullets above are what is claimed. Nothing more.
...
struct AccountStorage {
    ...
    bytes32  ownerPubKeyHash;              // SHA-256 of passkey public key
    uint64   nonce;                        // ERC-4337 standard nonce

    // ── tiering (Layer 4) ────────────────────────────────────────────
    uint128  hotLimitUsd;                  // user-set, default $500; floor $0
    uint128  hotBalanceDol;                // DOL currently in Hot
    uint128  hotBalanceUsdc;               // USDC currently in Hot
    uint128  vaultBalanceDol;              // DOL currently in Vault
    uint128  vaultBalanceUsdc;             // USDC currently in Vault
    ...
    // ── ops and policy ───────────────────────────────────────────────
    bool     paused;                       // emergency pause, owner-only
    bool     vaultTierEnabled;             // gate: off until guardians are set
}
...
Layout stability: the first four members (`ownerPubKeyHash`, `nonce`, `hotLimitUsd`, `hotBalanceDol`) are fixed across any future minor versions. New fields are appended to minimize storage-slot collisions during any future migration. A storage-layout test asserts these offsets in CI.
...
2. The operation's selector is in a **sponsorable-selectors allowlist**: `deposit`, `withdrawHot`, `initiateVaultWithdraw`, `cancelWithdraw`, `executeVaultWithdraw`, `initiateRecovery`, `approveRecovery`, `executeRecovery`, `addGuardian`, `removeGuardian`, `setThreshold`, `setHotLimit`. Operations with other selectors pay their own gas.
...
└─────────────────────┬────────────────────┘
        ...
        EntryPoint.validateUserOp() on Account:
          - check signature against ownerPubKeyHash
...
interface IDolVaultAccount {
    ...
    // ── Tiering policy ───────────────────────────────────────────────
    function setHotLimit(uint256 newLimitUsd) external;
    function setVaultTimelock(uint32 seconds_) external;  // ≥ 86400

    // ── Emergency ────────────────────────────────────────────────────
    function pause() external;     // owner-only; blocks withdrawals
...
| `setHotLimit` | Account owner | Any USD value ≥ 0 |
| `setVaultTimelock` | Account owner | `seconds_` ≥ 86400; change applies only to requests created *after* this call |
| `pause`, `unpause` | Account owner | — |

Note that `executeRecovery` and `executeVaultWithdraw` are callable by anyone after their respective timelocks. Making them permissionless means a user who is offline at the exact moment the timelock expires does not lose the transition — a third party (typically Dol's keeper, subject to paymaster rate limiting) can finalize. This is purely a liveness property; the authorization condition (timelock + threshold, or timelock alone) has already been satisfied.
...
3. Each guardian opens the link / notification:
     - Dashboard shows: "Your friend <name> requests recovery of
       their Dol Vault. New owner hash: <newOwnerPubKeyHash>."
     - The guardian signs the on-chain call `initiateRecovery` (if
       they are the first) or `approveRecovery` (if already initiated).
     - Gas is sponsored by the paymaster under the guardian-approval
       selector allowlist.
...
The tier split is a single user-settable threshold `hotLimitUsd`, stored on-chain in the Account contract:
...
- **Default at onboarding:** $500. Chosen as a round number on the order of a user's weekly discretionary spending; easy to adjust.
- **Ceiling:** no hard cap. A user with a $1,000,000 account who wants $50,000 in Hot may do so, knowing the risk.
- **Change authentication:** passkey-only. Raising `hotLimitUsd` above the current Vault balance is simply a tier change, not a withdrawal, so there is no timelock on the change itself. This is a deliberate policy choice: constraining tier adjustments with a timelock would add UX friction without meaningful security gain (a thief with the passkey who raises the Hot limit then withdraws from Hot is exactly the same attack surface as withdrawing from the then-unprotected Vault in the first place).

### 8.3 Automatic rebalancing

Because DOL and USDC prices fluctuate (slightly for USDC; per-strategy for DOL), the notional USD value of Hot can drift above or below the threshold over time. Dol Vault rebalances automatically within a hysteresis band to avoid thrashing:

- **Trigger up:** `hot_usd > hotLimitUsd × 1.10`. Move the excess DOL (first, preferring DOL over USDC for Vault storage) into the Vault tier. On-chain, this is a same-account state transition; no actual token transfer occurs externally.
- **Trigger down:** `hot_usd < hotLimitUsd × 0.50`. Move from Vault to Hot until Hot is restored to a floor (default: `hotLimitUsd × 0.80`).

The 10% hysteresis band prevents edge-case thrashing where a $1 market move would repeatedly flip the split. It is not user-configurable in v0.1.

Rebalancing triggers on:

- Every user-initiated action that changes balances (deposit, Hot withdrawal, Vault execute). The contract checks and rebalances inline if needed.
- A daily keeper sweep (off-chain, operated by Dol). The keeper enumerates accounts whose last balance-change event is old and whose current USD valuation would trigger rebalance. It submits UserOps on behalf of the accounts. The keeper cannot move funds out of an account — only execute the internal-tier-transfer functions, which are public and authorization-free at the contract level (tier transfers do not change external state).
...
**v0.1 choice:** treat **DOL = 1 USDC** at the vault level for tiering purposes. This is defensible because:

1. DOL is a receipt token for a USDC-denominated strategy; the protocol's own redemption logic uses 1:1 parity at mint.
2. Short-term deviations from 1:1 (if any) are smaller than the 10% hysteresis band.
3. It avoids the entire oracle-trust question for a feature (tier allocation) that does not move funds out of the user's account.

When Dol introduces a NAV-aware redemption rate (an ERC-7540-style async redeem with a variable exchange rate), the oracle question reopens. v0.1 pins 1:1; the decision is revisited in v0.2 with an Open Question in §17.

USDC is always valued at $1. USD-peg breaks are a systemic event larger than Dol Vault's scope to address.

### 8.5 Keeper operational model

The daily rebalance sweep is operated by Dol as a batched keeper job. Properties:

- **Authority.** The keeper submits UserOperations that call the account's internal tier-transfer function, which is permissionless at the contract level (it does not authorize external transfers). The keeper cannot withdraw user funds under any circumstance.
- **Gas.** The keeper's UserOps go through the paymaster under the "keeper" selector allowlist. This budget is bounded to prevent a runaway keeper from draining the paymaster.
- **Failure handling.** If the keeper is down or falls behind, the tier split drifts toward stale values. The next user action (deposit or withdrawal) will trigger inline rebalancing; the user is never blocked.
- **Decentralization.** The keeper is centralized in v0.1 for operational simplicity. A future version can open the keeper role to anyone (permissionless) with a small bounty paid from the paymaster.

### 8.6 Failure modes

| Assumption | Violation | Effect |
|---|---|---|
| DOL = 1 USDC for tiering purposes | DOL trades significantly off-peg for an extended period | Tier allocation is miscomputed; user's "Vault" could become too small or too large in USD terms. Mitigation: v0.2 introduces an oracle-backed tier valuation; v0.1 accepts this as Hot/Vault tier allocation is not a solvency mechanism. |
...
| Hysteresis band prevents thrashing | Adversary deliberately oscillates a user's balance to cause many paymaster calls | Wasted paymaster spend. Mitigation: rate limits (§6.4) apply to the keeper as well. |
| `hotLimitUsd` is a sane user choice | User sets it to an absurd value (e.g., $1 trillion) | Vault is effectively disabled; user has no timelock protection. Mitigation: UI warning at extreme values; user's explicit choice. |
| Tier-transfer functions cannot exfiltrate funds | Bug allows a malicious call to transfer to an external address | Catastrophic. Mitigation: audit scope; tight unit tests; tier-transfer functions should never construct an `external call` to arbitrary addresses. |
...
### 9.6 Attack scenario: user is unreachable for ≥ 24 h

This is the explicit residual risk of the design. A user who:

- Has had their passkey compromised,
- Has no working notification channel,
- And does not log into the dashboard for 24+ hours,

will have their Vault drained at the end of the timelock window. This is documented openly in the marketing guardrails and the ToS. The mitigation Dol offers is:

1. Strongly recommending that users enroll at least one notification channel (email at minimum) during onboarding.
2. Making the user's timelock duration easy to extend before a travel window or extended unreachability.
3. Guardian-assisted recovery (§7), which can still rotate ownership even after the Vault drain, so the *next* attack is prevented — this is not a cure for the initial drain.

A future version may introduce optional "vacation mode" (user-set max timelock extension until an explicit re-activation), but this is not in v0.1.
...
| Notification channels work | Push disabled, email spam-filtered | User does not notice pending request. Mitigation: dashboard banner on next login; multi-channel delivery. |
...
1. *Welcome.* "Your Dol Vault is a secure home for your DOL and USDC. It uses Face ID instead of a seed phrase and lets friends help you recover access if you ever lose your device. Create your vault in 30 seconds."
   - Primary button: "Create with Face ID."
...
- Dashboard calls Privy SDK to register a new discoverable passkey.
...
**Expected time:** 30–45 seconds including the OS biometric prompt.

**Friction point + justification:** the biometric prompt is the only non-trivial interaction. It is necessary and users understand it from other apps.
...
| Passkey stolen, Vault enabled, user **offline ≥ 24 h** | Vault timelock expires; attacker executes Vault withdrawal. Guardian path can rotate *next* owner, but this withdrawal is done. | Hot + Vault both lost; documented residual risk. |
...
| `k` guardians collude, user **offline ≥ 24 h** | Recovery completes; vault ownership transferred to attacker. | Full loss; documented residual risk. |
...
2. **Decide to pause.** If the contract has per-account `pause()` (owner-only) — the per-user, self-pause — individual users can pause their own account through the dashboard. There is *no* global pause affecting all accounts: a global pause was explicitly rejected because it would require a central authority with power over every user's vault.
3. Dol publishes a plain-language advisory on a non-dashboard channel (status page, Twitter) explaining the issue and what users can do.
...
- [ ] Privacy policy explicit about what Dol stores (passkey public-key hash only; guardian addresses; *not* guardian names unless user opts in).
...
### 17.1 Open questions (v0.1 resolved before v1.0)
...
- **Oracle selection for tiering when DOL/USDC deviates.** v0.1 assumes 1:1. v0.2 must decide between a protocol-NAV oracle, a TWAP, or a Chainlink feed (if one exists by then).
- **Dashboard form-factor.** PWA vs native app. PWA is lighter to ship and update; native offers better push-notification reliability. Decision deferred to the v0.1 → v1.0 transition.
- **SMS notifications.** Operational cost and delivery reliability across markets. v1 scope likely excludes SMS; v2 revisits.
- **Keeper sweep frequency.** Daily is proposed; busier usage may push toward hourly. To be calibrated.
- **Emergency-pause scope.** Per-account self-pause is in v0.1. A global (Dol-team-controlled) pause could be argued for Class A incidents but compromises non-custodial posture; explicitly deferred.
- **`setHotLimit` threshold for triggering extra UX guardrails.** Should setting a very large Hot limit trigger a confirmation ceremony (e.g., a 1-hour delay)? Open.
...
- Dol Vault's 24-hour timelock assumes a user who checks notifications at least every 24 hours during active account management. An always-unreachable user cannot be fully protected.
...
*This document is a research engineering specification of the Dol project for Dol Vault consumer custody. Does not constitute a security audit or certification. v0.1 is pre-audit; expect changes after external review.*
